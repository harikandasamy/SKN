...
def main():
    ...
    with st.expander("Authentication"):
        ...
        twofactor = st.text_input("Two Factor Authentication", key="twofactor")
        
        if st.button("Authenticate"):
            status, msg, code = validate_user(19, search_term, twofactor if twofactor else -1)
            if status <= 2:
                st.error(msg)
                if status == 1:
                    email_result = send_email()
                    st.info(f"Code sent via email: {code}. Email status: {email_result}")
            else:
