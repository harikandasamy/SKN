...
def process_request(callno, types=None):
    ...
    search_term_val = st.text_input("search_term", value=st.session_state.get("search_term", ""), key="search_term")
    twofactor_val = st.number_input(
    "twofactor",
    value=max(0, min(int(st.session_state.get("twofactor", 0) or 0), 999999)),
    min_value=0,
    max_value=999999,
    step=1,
    key="twofactor"
